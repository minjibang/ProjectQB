...
public class CustomerController {
	...
	*/
	...
	public String notices(String pg , String f , String q , Model model) {
		...
		//return "notice.jsp";
		return "customer.notice"; //폴더경로.파일이름
	}
	...
	public String noticeReg(Notice n, HttpServletRequest request , Principal principal) throws IOException, ClassNotFoundException, SQLException {
			                                                        //생성된 Principal 객체가 있다면 객체의 주소 자동 주입
		   //다중 파일 업로드
		   //view 코드 (JSP)
		   ...
		   //security 설정 처리에서 인증사용자 정보 얻기
		   //인증과 권한 자동처리 (내부적으로 session  , 별도의 객체를 사용)
		   /*
		   SecurityContext context = SecurityContextHolder.getContext();//
		   Authentication auth = context.getAuthentication();
		   UserDetails userinfo = (UserDetails)auth.getPrincipal();
		   System.out.println(userinfo.getAuthorities()); //권한 정보 ********
		   System.out.println(userinfo.getUsername()); //로그인한 ID 
		   System.out.println(userinfo.getPassword()); //
		   
		   n.setWriter(userinfo.getUsername()); //한줄 추가하면 글쓴이가 로그인한 ID 정보
		   */
